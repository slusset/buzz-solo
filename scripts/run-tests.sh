...
run_unit_tests() {
  ...
  # The Solo center: durable local relay + node CLI. Infra-free (in-process
  # servers on ephemeral ports), so it belongs in the unit lane.
  run_test_step "buzz-local-relay tests" \
    cargo test -p buzz-local-relay -- --nocapture
