...
fn do_sign(key_id: &str, status: &mut StatusWriter) -> Result<(), Error> {
    ...
    if let Some(ref oa_val) = oa {
        ...
        if parse_bip340_pubkey(&oa_val.0).is_err() {
            return Err(Error::Fatal(
...
fn do_verify(sig_file: &str, status: &mut StatusWriter) -> Result<(), Error> {
    ...
    let pk = parse_bip340_pubkey(&envelope.pk).map_err(|e| {
        write_errsig(status, Some(&envelope.pk));
        Error::VerifyFailed {
    ...
    let oa_result = if let Some(ref oa) = envelope.oa {
        ...
        if parse_bip340_pubkey(&oa.0).is_err() {
            write_errsig(status, Some(&envelope.pk));
            return Err(Error::VerifyFailed {
                pk: Some(envelope.pk),
...
/// Parse a 64-char hex string as a BIP-340 x-only public key, validating the
/// point is on the curve. Under nostr 0.44 `PublicKey::from_hex` is hex-decode
/// only — curve validation happens in `.xonly()`, so both steps are required.
fn parse_bip340_pubkey(hex_pk: &str) -> Result<PublicKey, String> {
    let pk = PublicKey::from_hex(hex_pk).map_err(|e| e.to_string())?;
...
fn verify_oa(agent_pk_hex: &str, oa: &(String, String, String)) -> bool {
    ...
    // Parse owner pubkey
    let owner_pk = match parse_bip340_pubkey(owner_pk_hex) {
        Ok(p) => p,
        Err(_) => {
